...
export default function DropReceivePage() {
  ...
  useEffect(() => {
    if (!valid) return;
    let alive = true;
    getDropShare(transferId).then((value) => { if (alive) setShare({ ...value, id: transferId }); }).catch((cause) => { if (alive) setError(cause?.message || "This transfer is unavailable."); });
    return () => { alive = false; };
  }, [transferId, valid]);
  ...
  return (
    <div className="flex min-h-[100svh] items-center justify-center px-[10px] pb-[44px] pt-[60px] [&>*]:pointer-events-auto lg:justify-start lg:p-0 lg:pl-32">
      {share ? <DropReceiveCard share={share} /> : error ? (
